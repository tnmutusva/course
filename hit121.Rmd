...
```{r echo=FALSE}
library(readxl)
cw121final<- read_excel("cw121finalxx.xlsx")
library(DT)
datatable(cw121final, rownames = FALSE,caption = 'For quick access, type your surname in the search box.',  options = list(order = list(list(1, 'asc')),pageLength = 100, scrollX=T,columnDefs=list(list(targets=7,visible=FALSE),list(className = 'dt-center', targets = 1:8))) ) %>%
  formatStyle(
    'Quiz_1',
  ...
  formatStyle(
    'CW',
    color = styleInterval(c(49.5, 75), c('red', 'black', 'green')), fontWeight = 'bold') 
```
