...
```{r echo=FALSE}
cw121c<-merge(cw122,cw122b,by.x="Reg",by.y="Reg", no.dups = TRUE,all=TRUE)

library(DT)
datatable(cw121c, rownames = FALSE,caption = 'For quick access, type your surname in the search box.',  options = list(order = list(list(1, 'asc')),pageLength = 5, scrollX=T,columnDefs=list(list(targets=3,visible=FALSE),list(targets=5,visible=FALSE),list(targets=6,visible=FALSE),list(targets=7,visible=FALSE),list(className = 'dt-center', targets = 2:8))),colnames= c("Registration","Surname","Department","4","Quiz 1","6","7","8","Quiz 2")) %>%
  formatStyle(
    'Quiz_1',
